...
Note that we're <B>not</B> using the <B>actual backtick character</B> below, as such. It has been substituted with the HTML backprime character, just for this example. That's because the three backticks themselves are difficult to render correctly within a code block!

> aside positive
> On an English (US) keyboard, the <B>backtick</B> (`&#96;`) character is located at top left, on the same key as the <B>tilde</B> (`~`) character. Worth noting is that the <I>proper</I> name for a code block is `fenced code block` and that each set of <B>three backticks</B> is called a `fence`
...
Add the following to your codelab file:
```
![YOUR_OWN_ALT_TEXT](assets/YOUR_OWN_FILENAME.EXT)
```
